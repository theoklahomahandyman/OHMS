...
class TestPurchaseModel(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.purchase = Purchase.objects.create(supplier=cls.supplier, supplier_address=cls.address, tax=6.83, date=timezone.now().date())
        ...
        cls.purchase_tool = PurchaseTool.objects.create(purchase=cls.purchase, inventory_item=cls.tool, quantity=26, cost=854.39)
    ...
    def test_purchase_material_save(self):
        ...
        purchase_material = PurchaseMaterial.objects.create(purchase=self.purchase, inventory_item=self.material, quantity=10, cost=200.0)
    ...
    def test_purchase_material_save_zero_quantity(self):
        initial_quantity = self.material.available_quantity
        PurchaseMaterial.objects.create(purchase=self.purchase, inventory_item=self.material, quantity=0, cost=200.0)
        self.material.refresh_from_db()
    ...
    def test_purchase_tool_save(self):
        ...
        purchase_tool = PurchaseTool.objects.create(purchase=self.purchase, inventory_item=self.tool, quantity=10, cost=200.0)
    ...
    def test_purchase_tool_save_zero_quantity(self):
        initial_quantity = self.tool.available_quantity
        PurchaseTool.objects.create(purchase=self.purchase, inventory_item=self.tool, quantity=0, cost=200.0)
        self.tool.refresh_from_db()
...
class TestPurchaseSerializer(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.negative_data = {'supplier': cls.supplier.pk, 'supplier_address': cls.address.pk, 'tax': -6.78, 'date': timezone.now().date(), 'uploaded_images': [cls.receipt]}
        cls.valid_data = {'supplier': cls.supplier.pk, 'supplier_address': cls.address.pk, 'tax': 6.78, 'date': timezone.now().date(), 'uploaded_images': [cls.receipt]}
    ...
    ''' Test purchase serializer create override '''
    ...
    ''' Test purchase serializer update override '''
...
''' Tests for purchase material serializer '''
class TestPuchaseMaterialSerializer(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.material = Material.objects.create(name='material', size='2 inch X 4 inch X 8 feet')
        cls.purchase_material = PurchaseMaterial.objects.create(purchase=cls.purchase, inventory_item=cls.material, quantity=10, cost=100.0)
        cls.empty_data = {'purchase': '', 'inventory_item': '', 'quantity': '', 'cost': ''}
        cls.negative_data = {'purchase': cls.purchase.pk, 'inventory_item': cls.material.pk, 'quantity': -10, 'cost': -73.29}
        cls.valid_data = {'purchase': cls.purchase.pk, 'inventory_item': cls.material.pk, 'quantity': 10, 'cost': 73.29}
...
''' Tests for purchase tool serializer '''
class TestPuchaseToolSerializer(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.purchase_tool = PurchaseTool.objects.create(purchase=cls.purchase, inventory_item=cls.tool, quantity=10, cost=100.0)
        cls.empty_data = {'purchase': '', 'inventory_item': '', 'quantity': '', 'cost': ''}
        cls.negative_data = {'purchase': cls.purchase.pk, 'inventory_item': cls.tool.pk, 'quantity': -10, 'cost': -73.29}
        cls.valid_data = {'purchase': cls.purchase.pk, 'inventory_item': cls.tool.pk, 'quantity': 10, 'cost': 73.29}
...
class TestPurchaseView(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.supplier = Supplier.objects.create(name='supplier')
        cls.address = SupplierAddress.objects.create(supplier=cls.supplier, street_address='123 Test Street', city='City', state='State', zip=12345)
        cls.purchase = Purchase.objects.create(supplier=cls.supplier, supplier_address=cls.address, tax=6.83, date=timezone.now().strftime('%Y-%m-%d'))
        ...
        cls.negative_data = {'supplier': cls.supplier.pk, 'supplier_address': cls.address.pk, 'tax': -6.78, 'date': timezone.now().strftime('%Y-%m-%d')}
        cls.create_data = {'supplier': cls.supplier.pk, 'supplier_address': cls.address.pk, 'tax': 6.78, 'date': timezone.now().date(), 'uploaded_images': [SimpleUploadedFile(name=cls.receipt_path, content=open(find(cls.receipt_path), 'rb').read(), content_type='image/jpg')]}
        cls.update_data = {'supplier': cls.supplier.pk, 'supplier_address': cls.address.pk, 'tax': 10.29, 'date': timezone.now().strftime('%Y-%m-%d')}
    ...
    def test_get_purchase_success(self):
        ...
        self.assertEqual(Decimal(response.data['total']), self.purchase.total)
    ...
    def test_create_purchase_success(self):
        ...
        purchase = Purchase.objects.filter(supplier=self.create_data['supplier'], supplier_address=self.create_data['supplier_address'], date=self.create_data['date'], tax=self.create_data['tax'])
...
''' Tests for purchase material view '''
class TestPurchaseMaterialView(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.empty_data = {'purchase': '', 'inventory_item': '', 'quantity': '', 'cost': ''}
        cls.negative_data = {'purchase': cls.purchase.pk, 'inventory_item': cls.material.pk, 'quantity': -10, 'cost': -73.29}
        cls.create_data = {'purchase': cls.purchase.pk, 'inventory_item': cls.material.pk, 'quantity': 10, 'cost': 73.29}
        cls.update_data = {'purchase': cls.purchase.pk, 'inventory_item': cls.material.pk, 'quantity': 15, 'cost': 137.42}
    ...
    def test_get_purchase_material_success(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['inventory_item'], self.purchase_material.inventory_item.pk)
    ...
    def test_create_purchase_material_success(self):
        ...
        purchase_material = PurchaseMaterial.objects.filter(purchase=self.create_data['purchase'], inventory_item=self.create_data['inventory_item'], quantity=self.create_data['quantity'], cost=self.create_data['cost'])
    ...
    def test_delete_purchase_material_success(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        self.assertEqual(PurchaseMaterial.objects.filter(purchase=self.purchase_material.purchase, inventory_item=self.purchase_material.inventory_item, quantity=self.purchase_material.quantity, cost=self.purchase_material.cost).count(), 0)

''' Tests for purchase new material view '''
class TestPurchaseNewMaterialView(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.address = SupplierAddress.objects.create(supplier=cls.supplier, street_address='123 Test Street', city='City', state='State', zip=12345)
        cls.purchase = Purchase.objects.create(supplier=cls.supplier, supplier_address=cls.address, tax=6.83, date=timezone.now().date())
        cls.material = Material.objects.create(name='material', size='2 inch X 4 inch X 8 feet', description='test')
        cls.purchase_material = PurchaseMaterial.objects.create(purchase=cls.purchase, inventory_item=cls.material, quantity=10, cost=100.0)
    ...
    ''' Test new material empty material data '''
    ...
    ''' Test new material short material data '''
    ...
    ''' Test new material empty purchase data '''
    ...
    ''' Test new material long purchase data '''
    ...
    ''' Test new material negative purchase data '''
    ...
    def test_new_material_success(self):
        ...
        self.assertTrue(PurchaseMaterial.objects.filter(purchase=self.purchase, inventory_item__name=self.valid_data['name'], quantity=self.valid_data['quantity'], cost=self.valid_data['cost']).exists())

    ''' Test existing material success '''
    def test_existing_material_success(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.post(self.url(self.purchase.pk), data=self.existing_data)
        self.purchase_material.refresh_from_db()
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertTrue(PurchaseMaterial.objects.filter(purchase=self.purchase, inventory_item__name=self.existing_data['name'], quantity=self.existing_data['quantity'], cost=self.existing_data['cost']).exists())
...
class TestPurchaseToolView(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.empty_data = {'purchase': '', 'inventory_item': '', 'quantity': '', 'cost': ''}
        cls.negative_data = {'purchase': cls.purchase.pk, 'inventory_item': cls.tool.pk, 'quantity': -10, 'cost': -73.29}
        cls.create_data = {'purchase': cls.purchase.pk, 'inventory_item': cls.tool.pk, 'quantity': 10, 'cost': 73.29}
        cls.update_data = {'purchase': cls.purchase.pk, 'inventory_item': cls.tool.pk, 'quantity': 15, 'cost': 137.42}
        cls.patch_data = {'quantity': 16}
    ...
    def test_get_purchase_tool_success(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['inventory_item'], self.purchase_tool.inventory_item.pk)
    ...
    def test_create_purchase_tool_success(self):
        ...
        purchase_tool = PurchaseTool.objects.filter(purchase=self.create_data['purchase'], inventory_item=self.create_data['inventory_item'], quantity=self.create_data['quantity'], cost=self.create_data['cost'])
    ...
    def test_delete_purchase_tool_success(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        self.assertEqual(PurchaseTool.objects.filter(purchase=self.purchase_tool.purchase, inventory_item=self.purchase_tool.inventory_item, quantity=self.purchase_tool.quantity, cost=self.purchase_tool.cost).count(), 0)

''' Tests for purchase new tool view '''
class TestPurchaseNewToolView(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.address = SupplierAddress.objects.create(supplier=cls.supplier, street_address='123 Test Street', city='City', state='State', zip=12345)
        cls.purchase = Purchase.objects.create(supplier=cls.supplier, supplier_address=cls.address, tax=6.83, date=timezone.now().date())
        cls.tool = Tool.objects.create(name='tool', description='test')
        cls.purchase_tool = PurchaseTool.objects.create(purchase=cls.purchase, inventory_item=cls.tool, quantity=10, cost=100.0)
    ...
    ''' Test new tool empty purchase data '''
    ...
    ''' Test new tool long purchase data '''
    ...
    ''' Test new tool negative purchase data '''
    ...
    def test_new_tool_success(self):
        ...
        self.assertTrue(PurchaseTool.objects.filter(purchase=self.purchase, inventory_item__name=self.valid_data['name'], quantity=self.valid_data['quantity'], cost=self.valid_data['cost']).exists())

    ''' Test existing tool success '''
    def test_existing_tool_success(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.post(self.url(self.purchase.pk), data=self.existing_data)
        self.purchase_tool.refresh_from_db()
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertTrue(PurchaseTool.objects.filter(purchase=self.purchase, inventory_item__name=self.existing_data['name'], quantity=self.existing_data['quantity'], cost=self.existing_data['cost']).exists())
